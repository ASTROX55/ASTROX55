...
function getReconnectDelay() {
  const baseDelay = Math.max(5000, Number(config.utils['auto-reconnect-delay']) || 10000);
  const maxDelay = Math.max(baseDelay, Number(config.utils['max-reconnect-delay']) || 300000);

  // Exponential backoff with a 5-minute ceiling. This keeps retrying
  // during Aternos queue/outage periods without hammering the server.
  const attempt = Math.max(0, botState.reconnectAttempts - 1);
...
function createBot() {
  ...
  // Invalidate listeners belonging to the previous bot instance.
  ...
  console.log(`[Bot] Creating bot instance (generation ${generation})...`);
  ...
  try {
    bot = mineflayer.createBot({
      ...
      hideErrors: false,
      // Detect dead/stalled connections, but don't disconnect normal idle AFK sessions.
      checkTimeoutInterval: 120000
    });

    bot.loadPlugin(pathfinder);

    // If Aternos is queued/unavailable and we never receive spawn,
    // schedule a retry instead of getting stuck forever.
    connectionTimeout = setTimeout(() => {
      if (generation !== botGeneration || shuttingDown) return;
      if (!spawned && !botState.connected) {
        console.log('[Bot] No spawn after 90s. Treating connection as failed.');
        try { bot.end(); } catch (_) {}
    ...
    bot.once('spawn', () => {
      ...
      setTimeout(() => {
        if (generation === botGeneration && bot && botState.connected) {
          try { bot.chat('/gamerule sendCommandFeedback false'); } catch (_) {}
        }
      }, 3000);
    ...
    bot.on('error', (err) => {
      ...
      // Mineflayer normally follows this with 'end'. If it doesn't,
      // schedule a reconnect so the bot cannot remain stuck indefinitely.
      if (config.utils['auto-reconnect'] && !botState.connected) {
...
function scheduleReconnect(reason = 'unknown') {
  if (shuttingDown || !config.utils['auto-reconnect']) return;
  ...
  console.log(
    `[Bot] Reconnect scheduled in ${Math.ceil(delay / 1000)}s ` +
    `(attempt #${botState.reconnectAttempts}, reason: ${reason})`
  );
...
process.on('uncaughtException', (err) => {
  ...
  if (config.utils['auto-reconnect'] && !shuttingDown) {
    clearAllIntervals();
    setTimeout(() => scheduleReconnect('uncaughtException'), 1000);
  }
});
...
process.on('SIGTERM', () => {
  shuttingDown = true;
  console.log('[System] SIGTERM received. Shutting down cleanly.');
